...
class Reinforce(tf.keras.Model):
    ...
    def loss(self, states, actions, discounted_rewards):
        ...
        pred = self.call(states)

        probabilities = []
        for i in range(len(pred)):
            probabilities.append(pred[i][actions[i]])

        return -1 * tf.reduce_sum(tf.math.log(tf.convert_to_tensor(probabilities)) * discounted_rewards)
